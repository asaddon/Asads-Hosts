...
def write_opening_header(final_file, **header_params):
    ...
    if header_params["extensions"]:
        if len(header_params["extensions"]) > 1:
            write_data(
                final_file,
                "# Title: Asads-Hosts with the {0} and {1} extensions\n#\n".format(
            ...
            write_data(
                final_file,
                "# Title: Asads-Hosts with the {0} extension\n#\n".format(
                    ", ".join(header_params["extensions"])
                ),
            )
    else:
        write_data(final_file, "# Title: Asads-Hosts\n")

    write_data(
        final_file,
        "# Date: " + time.strftime("%d %B %Y %I:%M:%S %p (%Z)") + "\n",
    )
